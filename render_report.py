...
async function startEvaluation(){if(evaluationActive){evaluationActive=false;if(pollTimer)clearInterval(pollTimer);workflowStop();$("start").textContent="▶ 启动评测";$("connection").textContent="评测已暂停";toast("实时评测已暂停");return}const fileMode=location.protocol==="file:";if(fileMode&&!mockReplay){toast("请通过本地演示服务启动评测");return}evaluationActive=true;$("start").textContent="Ⅱ 暂停评测";$("connection").textContent=mockReplay?"Mock 回放中":"实时分析中";toast("已启动实时评测");if(mockReplay){runMockEvents()}else if(!fileMode){await pollReport();pollTimer=setInterval(pollReport,Number(DATA.presentation.poll_interval_ms||650))}}
