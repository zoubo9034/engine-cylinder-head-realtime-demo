...
def _public_item(item: Mapping[str, Any]) -> dict[str, Any]:
    ...
    # visible before this item reaches the completed state.
...
def public_projection(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "scope": {"active_item_count": len(items)},
        "items": items,
        "events": events,
    }
...
.metrics{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px}.metric{background:linear-gradient(150deg,#12263b,#0d192b);border:1px solid var(--line);border-radius:16px;padding:14px 16px;box-shadow:var(--shadow)}.metric-label{color:var(--muted);font-size:12px}.metric-value{font-size:20px;font-weight:750;margin-top:5px}.metric-value.cyan{color:var(--cyan)}.metric-value.green{color:var(--green)}
...
const labels = {high:"高风险",medium:"中等",stable:"稳定"};
...
function doneCount(){return state.filter(x=>["证据已绑定","已完成评分","待人工确认"].includes(x.status)).length}
...
function cardMarkup(item,i){const evaluation=item.status==="已完成评分"?`<div class="target"><span class="target-label">评分结论</span>${esc(item.realtime_target)}</div>`:"";return `<article class="item-card ${i===current?"current":""} ${item.status==="待人工确认"?"review":""}" data-difficulty="${item.difficulty}"><div class="card-head"><div class="item-id"><span class="item-no">${item.item_number}</span><div><div class="item-title">${esc(item.display_name)}</div><span class="difficulty ${item.difficulty}">${labels[item.difficulty]||item.difficulty}</span></div></div><span class="status-badge ${cls(item.status)}">${esc(item.status)}</span></div>${evaluation}<div class="binding-line"><span>现场时间</span><strong>${esc(item.binding.live_timestamp||"等待")}</strong><span>·</span><span>置信度</span><strong>${item.binding.time_confidence!=null?Math.round(Number(item.binding.time_confidence)*100)+"%":"—"}</strong></div>${evidenceMarkup(item)}</article>`}
function renderCards(){document.querySelectorAll(".item-card").forEach(x=>x.remove()); const visible=state.map((item,i)=>({item,i})).filter(x=>filter==="all"||x.item.difficulty===filter); $("cards").innerHTML=visible.map(x=>cardMarkup(x.item,x.i)).join("");}
function render(){
  const item=state[current], complete=doneCount(), bound=state.reduce((n,x)=>n+(x.binding.evidence||[]).length,0);
  $("title").textContent=DATA.title; $("footer").textContent=DATA.presentation.footer; $("done").textContent=`${complete}/${state.length}`; $("bound").textContent=String(bound); $("heroProgress").textContent=`${complete} / ${state.length}`; $("progressBar").style.width=`${Math.round(complete/state.length*100)}%`;
  $("phase").textContent=item.status==="待开始"?DATA.presentation.initial_state:item.status; $("quality").textContent=item.status==="待人工确认"?"需要人工确认":item.status==="证据生成中"?"证据生成中":complete?"证据链稳定":"等待有效画面"; $("heroKicker").textContent=`实时识别中 · 项目 ${item.item_number}`; $("heroTitle").textContent=item.display_name; const liveExplanation=item.binding.evidence_explanation&&item.binding.evidence_explanation!=="等待当前视频流中的有效证据。"?item.binding.evidence_explanation:"系统正在从视频流中定位操作对象，证据生成后会自动绑定到对应项目。"; $("heroText").textContent=item.status==="已完成评分"?(item.binding.evidence_explanation||item.realtime_target||"当前评分项已完成。"):liveExplanation;
...
function mergeView(view){
  ...
  view.items.forEach(incoming=>{
    ...
    if(incomingRevision>currentRevision||incomingEvidence>currentEvidence||incomingState!==currentState||hasEvidence){pending.push({index,incoming,finalState:displayState(incomingState)});pendingKeys.add(incoming.item_id)}
  });
  drainQueue();
}
async function present(update){
  ...
  item.status="证据生成中"; render(); await delay(item.difficulty==="high"?1250:850);
  if(!evaluationActive){pendingKeys.delete(update.incoming.item_id);return;}
...
async function runMockEvents(){
  for(const event of DATA.events||[]){if(!evaluationActive)break;await delay(Number(event.delay_ms||900));if(!evaluationActive)break;const patch=event.item_patch;if(patch)mergeView({items:[patch]});while(processing&&evaluationActive)await delay(120);}
}
...
async function resetReport(){
  ...
  try{const response=await fetch("/api/reset",{method:"POST"});if(!response.ok)throw new Error("reset failed");const view=await response.json();state=view.items.map((item,index)=>({...item,status:"待开始",index,binding:{...item.binding,evidence:[...(item.binding.evidence||[])]}}));current=0;$("start").textContent="▶ 启动评测";$("connection").textContent="视频流已连接";render();toast("已恢复初始评测状态")}
...
def main() -> int:
    ...
    if args.command == "template":
        _write_json(args.output, template_payload())
