...
def _public_video_evidence(
    evidence: Mapping[str, Any],
    *,
    mock_static_media: bool = False,
) -> dict[str, Any]:
    """Project one validated clip without exposing its local source path."""
    ...
    if source_url:
        src = source_url
    elif mock_static_media:
        src = f"../../mock-video-evidence/{quote(source_path, safe='/')}"
    else:
        src = f"../../api/evidence-media/{item_id}/{evidence_id}"
    return {
        "schema": VIDEO_EVIDENCE_SCHEMA,
        "evidence_id": evidence_id,
        "item_id": item_id,
        "kind": "video",
        "slot_ids": [str(value) for value in evidence.get("slot_ids", []) or []],
        "mime_type": "video/mp4",
        ...
        "confidence": evidence.get("confidence"),
        "caption": _clean_text(evidence.get("caption"), "当前项目的视频证据。", 220),
        "src": src,
    }
...
def _public_item(
    ...
    *,
    ...
    video_evidence = (
        _public_video_evidence(video_record, mock_static_media=mock_static_media)
        if evidence_media_mode == "video" and isinstance(video_record, Mapping)
        else None
    )
    evidence_ids = {entry["evidence_id"] for entry in evidence}
    video_slot_ids = set(video_evidence.get("slot_ids", []) if video_evidence else [])
    required_slots = []
    for slot in item.get("required_evidence_slots", []) or []:
        ...
        required_slots.append({
            ...
            "bound": (
                str(slot.get("status") or "") == "bound"
                or bool(evidence_ids.intersection(slot_evidence))
                or str(slot.get("slot_id") or "") in video_slot_ids
            ),
        })
...
def _public_presentation(presentation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence_media_mode": (
            str(presentation.get("evidence_media_mode") or "image")
            if str(presentation.get("evidence_media_mode") or "image") in EVIDENCE_MEDIA_MODES
            else "image"
        ),
...
def public_projection(payload: Mapping[str, Any]) -> dict[str, Any]:
    ...
    mock_static_media = payload.get("demo_mode") == "mock_live_stream" and evidence_media_mode == "video"
    items = [
        _public_item(item, evidence_media_mode, mock_static_media=mock_static_media)
    ...
    for event in payload.get("events", []) or []:
        ...
        if isinstance(event.get("item_patch"), Mapping):
            public_event["item_patch"] = _public_item(
                event["item_patch"],
                evidence_media_mode,
                mock_static_media=mock_static_media,
            )
...
.evidence-video-wrap{position:relative;overflow:hidden;border:1px solid var(--rule);border-radius:5px;background:#111b28;aspect-ratio:16/9}.lightbox .evidence-video-wrap{width:100%;align-self:center}.evidence-video{display:block;width:100%;height:100%;object-fit:contain;background:#111b28}.video-playback-status:not(:empty){position:absolute;left:10px;right:10px;bottom:42px;padding:7px 9px;border-radius:4px;background:rgba(26,35,50,.88);color:#fff;text-align:center;font-family:"JetBrains Mono","SFMono-Regular",monospace;font-size:9px;pointer-events:none}.evidence-video-meta{display:flex;justify-content:space-between;gap:8px;margin-top:7px;color:var(--ink-mute);font-family:"JetBrains Mono","SFMono-Regular",monospace;font-size:9px}.video-service-note{display:grid;place-items:center;min-height:150px;padding:18px;text-align:center;color:var(--ink-faint);font-family:"JetBrains Mono","SFMono-Regular",monospace;font-size:10px;background:var(--paper-warm)}.lightbox-video{flex:1;min-height:0;width:100%;object-fit:contain;background:#111b28;border-radius:4px}
...
<style>
.workflow-strip{position:static}
.live-layout{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;align-items:start;min-height:clamp(520px,calc(100vh - 265px),780px)}
.video-column{min-width:0;display:flex;flex-direction:column;gap:14px;position:sticky;top:92px;align-self:start}
.results-column{height:clamp(520px,calc(100vh - 265px),780px);min-width:0;min-height:0;display:flex;flex-direction:column}
.video-column .workflow-strip{width:100%;margin-bottom:0}
.video-column .hero{margin-bottom:0;align-items:stretch;flex-direction:column;gap:14px;padding:18px 20px}
.video-column .hero-side{min-width:0;text-align:left}
.video-column .hero p{max-width:none}
.video-column .toolbar{display:flex;flex-direction:column;align-items:stretch;gap:10px;margin-bottom:0;padding:12px 14px;border:1px solid var(--rule);border-radius:6px;background:var(--paper-cool)}
.video-column .filters,.video-column .controls{width:100%}
.video-column .controls .control{flex:1}
.video-column .difficulty-hint{padding:0}
.video-slot{position:relative;aspect-ratio:16/9;width:100%;margin-bottom:0;overflow:hidden;border:1px solid var(--rule);border-radius:8px;background:linear-gradient(135deg,#1a2332 0%,#253449 52%,#172331 100%);box-shadow:var(--shadow)}
...
.video-placeholder[hidden]{display:none}.webrtc-source-video{position:fixed;left:-4px;bottom:-4px;width:2px;height:2px;opacity:.001;pointer-events:none}
...
.workflow-strip{display:flex;align-items:center;gap:14px;margin-bottom:14px;padding:12px 14px;border:1px solid var(--rule);border-radius:6px;background:rgba(250,250,247,.96);box-shadow:0 4px 14px rgba(26,35,50,.05);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px)}
...
.cards{grid-template-columns:1fr;flex:1;min-height:180px;max-height:none;overflow-y:auto;overflow-x:hidden;padding:3px 6px 22px 2px;scroll-padding-block:32vh}
...
.follow-chip{align-self:flex-start;margin:0 0 10px;border:1px solid #c6d7ea;border-radius:6px;background:var(--vermilion-bg);color:var(--vermilion);padding:9px 12px;font-family:"JetBrains Mono","SFMono-Regular",monospace;font-size:10px;cursor:pointer}
.follow-chip[hidden]{display:none}.follow-chip:hover{border-color:var(--vermilion)}
...
@media(max-width:1050px){.live-layout{grid-template-columns:1fr;min-height:0}.video-column{position:static}.video-slot{position:sticky;top:82px;z-index:10}.results-column{height:auto;min-height:0}.cards{flex:none;max-height:62vh;min-height:220px}}
...
<body>
<div class="shell">
  <header class="topbar"><div class="brand"><div class="brand-mark">AI</div><div><div class="eyebrow">LIVE ANALYSIS CONSOLE</div><h1 id="title">实时智能实训分析</h1></div></div><div class="live-pill"><i class="pulse"></i><span id="connection">等待视频接入</span></div></header>
...
<div class="lightbox" id="lightbox" hidden role="dialog" aria-modal="true" aria-label="证据查看器"><div class="lightbox-panel"><button class="lightbox-close" id="lightbox-close" type="button" aria-label="关闭证据查看器">×</button><div id="lightbox-media" style="display:flex;flex:1;min-height:0"></div><div class="lightbox-meta" id="lightbox-meta"></div></div></div>
<script>
...
const LOCAL_WEBRTC_SAMPLE = "__LOCAL_WEBRTC_SAMPLE__";
...
let current = 0, activeAnalysisIndex = 0, evaluationActive = false, processing = false, pollTimer = null, filter = "all", pending = [], pendingKeys = new Set(), drawerIndex = null, newCardIndexes = new Set();
...
let liveReceiverPeer = null, liveSenderPeer = null, liveSourceVideo = null, liveSourceObjectUrl = null, liveRemoteStream = null;
...
function ensureRealtimeSurfaces(){
  const main=document.querySelector(".main"), hero=main&&main.querySelector(".hero"), toolbar=main&&main.querySelector(".toolbar"), cards=main&&main.querySelector("#cards");
  if(!main||!hero)return;
  let videoSlot=$("video-slot");
  if(!videoSlot){
    videoSlot=document.createElement("section");videoSlot.id="video-slot";videoSlot.className="video-slot";videoSlot.setAttribute("aria-label","实时视频窗口");
    videoSlot.innerHTML='<video id="live-video" data-video-slot="realtime" playsinline muted aria-label="实时视频画面"></video><div class="video-placeholder"><div class="video-placeholder-inner"><div class="video-placeholder-mark">LIVE</div><div class="video-placeholder-title">实时视频</div><div class="video-placeholder-note">等待视频源接入</div></div></div>';
  }
  let workflow=$("workflow-strip");
  if(!workflow){
    workflow=document.createElement("section");workflow.id="workflow-strip";workflow.className="workflow-strip";workflow.setAttribute("aria-label","后端模型工作状态");
    workflow.innerHTML='<div class="workflow-heading"><i></i><span>模型工作状态</span></div><div class="workflow-track" id="workflow-track" role="list">'+workflowStages.map(stage=>`<div class="workflow-stage pending" data-workflow-stage="${esc(stage.stage_id)}" role="listitem"><i class="workflow-light"></i><span>${esc(stage.label)}</span></div>`).join("")+'</div><span class="workflow-state" id="workflow-state" role="status" aria-live="polite">等待分析</span>';
  }
  let layout=main.querySelector(".live-layout");
  if(!layout){
    layout=document.createElement("div");layout.className="live-layout";main.append(layout);
  }
  let videoColumn=layout.querySelector(".video-column");
  if(!videoColumn){videoColumn=document.createElement("aside");videoColumn.className="video-column";layout.prepend(videoColumn)}
  let resultsColumn=layout.querySelector(".results-column");
  if(!resultsColumn){resultsColumn=document.createElement("section");resultsColumn.className="results-column";layout.append(resultsColumn)}
  videoColumn.append(videoSlot,workflow,hero);if(toolbar)videoColumn.append(toolbar);
  if(cards)resultsColumn.append(cards);
  let followChip=$("follow-chip");
  if(!followChip){followChip=document.createElement("button");followChip.id="follow-chip";followChip.className="follow-chip";followChip.type="button";followChip.hidden=true;followChip.addEventListener("click",jumpToLatestCompleted)}
  if(followChip.parentElement!==resultsColumn)resultsColumn.insertBefore(followChip,resultsColumn.firstChild);
  if(cards){cards.setAttribute("tabindex","0");cards.setAttribute("aria-label","评分项目列表")}
}
...
function workflowStart(itemIndex,totalMs){
  ...
  const advance=()=>{if(token!==workflowToken||workflowItemIndex!==itemIndex)return;workflowStageIndex=index;workflowSetVisual(index,"running");const wait=schedule[index];index+=1;if(index<schedule.length)workflowTimer=setTimeout(advance,reduced?Math.min(wait,80):wait);else workflowTimer=setTimeout(()=>{if(token===workflowToken&&workflowItemIndex===itemIndex){workflowStart(itemIndex,totalMs)}},reduced?80:wait)};
  advance();
}
...
function videoEvidenceMarkup(item,itemIndex,compact=false){const video=item.binding&&item.binding.video_evidence;if(!video)return `<div class="empty-note">当前项目仍在等待可用视频证据</div>`;if(location.protocol==="file:")return `<div class="video-service-note">视频证据需要通过本地演示服务查看</div>`;const meta=`${esc(video.caption||"视频证据")} · ${Number(video.width)||960}×${Number(video.height)||540} · ${Number(video.fps)||10}fps`;return `<div class="evidence-video-wrap"><video class="evidence-video" data-evidence-video="true" data-item-index="${itemIndex}" data-evidence-id="${esc(video.evidence_id)}" src="${esc(video.src||"")}" controls muted loop playsinline preload="${itemIndex===current?"metadata":"none"}" aria-label="${esc(video.caption||"视频证据")}"></video><div class="video-playback-status" aria-live="polite"></div></div>${compact?"":`<div class="evidence-video-meta"><span>${meta}</span><button class="evidence-ref" type="button" data-item-index="${itemIndex}" data-evidence-id="${esc(video.evidence_id)}">放大查看</button></div>`}`}
...
function updateFollowChip(){const chip=$("follow-chip");if(!chip)return;if(followLatest||pendingCompletedIndex===null){chip.hidden=true;chip.textContent="";return}const item=state[pendingCompletedIndex],label=item?`项目 ${item.item_number} 已完成`:"新项目已完成";chip.textContent=pendingCompletedCount>1?`${label} 等 ${pendingCompletedCount} 项 · 查看`:`${label} · 查看`;chip.hidden=false}
...
function requestLatestFocus(index){pendingCompletedIndex=index;pendingCompletedCount+=1;if(followLatest){pendingCompletedCount=0;current=index;render();focusLatestCompleted(index);return}updateFollowChip()}
...
function setLiveVideoState(state,label){const slot=$("video-slot"),placeholder=slot&&slot.querySelector(".video-placeholder");if(slot)slot.dataset.streamState=state;if(placeholder)placeholder.hidden=state==="connected";if(label&&$("connection"))$("connection").textContent=label;window.dispatchEvent(new CustomEvent("realtime-video-state",{detail:{state,label}}))}
function closeReceiverPeer(clearVideo=true){if(liveReceiverPeer){liveReceiverPeer.ontrack=null;liveReceiverPeer.onconnectionstatechange=null;liveReceiverPeer.close();liveReceiverPeer=null}if(clearVideo){const video=$("live-video");if(video){video.pause();video.srcObject=null}liveRemoteStream=null;setLiveVideoState("idle","等待视频接入")}}
function closeLocalSender(){if(liveSenderPeer){liveSenderPeer.close();liveSenderPeer=null}if(liveSourceVideo){liveSourceVideo.pause();liveSourceVideo.src="";liveSourceVideo.remove();liveSourceVideo=null}if(liveSourceObjectUrl){URL.revokeObjectURL(liveSourceObjectUrl);liveSourceObjectUrl=null}}
function closeLiveVideoInput(){closeLocalSender();closeReceiverPeer(true)}
function waitForIceGathering(peer,timeoutMs=5000){if(peer.iceGatheringState==="complete")return Promise.resolve();return new Promise(resolve=>{let timer=null;const finish=()=>{peer.removeEventListener("icegatheringstatechange",changed);if(timer)clearTimeout(timer);resolve()};const changed=()=>{if(peer.iceGatheringState==="complete")finish()};peer.addEventListener("icegatheringstatechange",changed);timer=setTimeout(finish,timeoutMs)})}
async function attachLiveStream(stream){const video=$("live-video");if(!video)throw new Error("实时视频窗口不存在");liveRemoteStream=stream;video.srcObject=stream;video.autoplay=true;video.muted=true;video.playsInline=true;setLiveVideoState("connecting","视频流连接中");try{await video.play()}catch(error){}return video}
async function createWebRTCAnswer(offer,rtcConfiguration={},keepLocalSender=false){if(typeof RTCPeerConnection!=="function")throw new Error("当前浏览器不支持 WebRTC 视频接入");if(!keepLocalSender)closeLocalSender();closeReceiverPeer(false);const peer=new RTCPeerConnection(rtcConfiguration||{});liveReceiverPeer=peer;peer.onicecandidate=event=>window.dispatchEvent(new CustomEvent("realtime-video-icecandidate",{detail:event.candidate?event.candidate.toJSON():null}));peer.ontrack=event=>{const stream=event.streams&&event.streams[0]?event.streams[0]:new MediaStream([event.track]);attachLiveStream(stream)};peer.onconnectionstatechange=()=>{const state=peer.connectionState;if(state==="connected")setLiveVideoState("connected","实时视频已连接");else if(state==="connecting"||state==="new")setLiveVideoState("connecting","视频流连接中");else if(state==="disconnected")setLiveVideoState("disconnected","视频流暂时中断");else if(state==="failed")setLiveVideoState("failed","视频流连接失败");else if(state==="closed"&&!liveReceiverPeer)setLiveVideoState("idle","等待视频接入")};await peer.setRemoteDescription(offer);const answer=await peer.createAnswer();await peer.setLocalDescription(answer);await waitForIceGathering(peer);return {type:peer.localDescription.type,sdp:peer.localDescription.sdp}}
async function acceptWebRTCOffer(offer,rtcConfiguration={}){return createWebRTCAnswer(offer,rtcConfiguration,false)}
async function addWebRTCIceCandidate(candidate){if(!liveReceiverPeer)throw new Error("WebRTC 接收端尚未初始化");await liveReceiverPeer.addIceCandidate(candidate||null)}
function waitForVideoReady(video,timeoutMs=8000){if(video.readyState>=2)return Promise.resolve();return new Promise((resolve,reject)=>{let timer=null;const cleanup=()=>{video.removeEventListener("loadeddata",ready);video.removeEventListener("error",failed);if(timer)clearTimeout(timer)};const ready=()=>{cleanup();resolve()};const failed=()=>{cleanup();reject(new Error("视频样本加载失败"))};video.addEventListener("loadeddata",ready,{once:true});video.addEventListener("error",failed,{once:true});timer=setTimeout(()=>{cleanup();reject(new Error("视频样本加载超时"))},timeoutMs)})}
async function startLocalWebRTCSource(sourceUrl=LOCAL_WEBRTC_SAMPLE){if(!sourceUrl)throw new Error("缺少视频源");if(typeof RTCPeerConnection!=="function")throw new Error("当前浏览器不支持 WebRTC 视频接入");closeLocalSender();const resolvedUrl=new URL(sourceUrl,document.baseURI);let playableUrl=resolvedUrl.href;if(/^https?:$/.test(resolvedUrl.protocol)){try{const response=await fetch(playableUrl,{cache:"force-cache"});if(response.ok){liveSourceObjectUrl=URL.createObjectURL(await response.blob());playableUrl=liveSourceObjectUrl}}catch(error){}}const source=document.createElement("video");source.className="webrtc-source-video";source.muted=true;source.loop=true;source.playsInline=true;source.preload="auto";source.src=playableUrl;source.setAttribute("aria-hidden","true");document.body.append(source);liveSourceVideo=source;try{source.load();await waitForVideoReady(source);await source.play();const capture=source.captureStream||source.mozCaptureStream;if(typeof capture!=="function")throw new Error("当前浏览器不支持视频流采集");const stream=capture.call(source,10),track=stream.getVideoTracks()[0];if(!track)throw new Error("视频源未产生画面轨道");const sender=new RTCPeerConnection({iceServers:[]});liveSenderPeer=sender;sender.addTrack(track,stream);await sender.setLocalDescription(await sender.createOffer());await waitForIceGathering(sender);const answer=await createWebRTCAnswer({type:sender.localDescription.type,sdp:sender.localDescription.sdp},{iceServers:[]},true);await sender.setRemoteDescription(answer);return {senderState:sender.connectionState,receiverState:liveReceiverPeer&&liveReceiverPeer.connectionState}}catch(error){closeLocalSender();setLiveVideoState("failed","视频流连接失败");throw error}}
function liveVideoInputState(){const video=$("live-video"),track=liveRemoteStream&&liveRemoteStream.getVideoTracks()[0];return {receiverConnectionState:liveReceiverPeer&&liveReceiverPeer.connectionState||"closed",senderConnectionState:liveSenderPeer&&liveSenderPeer.connectionState||"closed",hasRemoteStream:Boolean(video&&video.srcObject),trackState:track&&track.readyState||"none",videoWidth:Number(video&&video.videoWidth||0),videoHeight:Number(video&&video.videoHeight||0),currentTime:Number(video&&video.currentTime||0)}}
window.realtimeVideoInput=Object.freeze({acceptOffer:acceptWebRTCOffer,addIceCandidate:addWebRTCIceCandidate,close:closeLiveVideoInput,startLocalSample:startLocalWebRTCSource,getState:liveVideoInputState});
async function videoServiceReady(){if(evidenceMediaMode!=="video"||location.protocol==="file:")return true;const video=embeddedVideoEvidence(),src=String(video&&video.src||"");if(!src||/^https?:\/\//i.test(src))return true;try{const response=await fetch(new URL(src,document.baseURI),{method:"HEAD",cache:"no-store"});return response.ok}catch(error){return false}}
async function startEvaluation(){if(evaluationActive){evaluationActive=false;if(pollTimer)clearInterval(pollTimer);workflowStop();pauseEvidencePlayers();$("start").textContent="▶ 启动评测";$("connection").textContent="评测已暂停";toast("实时评测已暂停");return}const fileMode=location.protocol==="file:";if(fileMode&&(evidenceMediaMode==="video"||!mockReplay)){toast("请通过本地演示服务启动视频证据展示");return}if(!await videoServiceReady()){$("connection").textContent="视频服务未就绪";toast("请检查视频证据服务");return}evaluationActive=true;$("start").textContent="Ⅱ 暂停评测";$("connection").textContent="实时分析中";toast("已启动实时评测");if(mockReplay){if(evidenceMediaMode==="video")startLocalWebRTCSource().catch(()=>{});runMockEvents()}else if(!fileMode){await pollReport();pollTimer=setInterval(pollReport,Number(DATA.presentation.poll_interval_ms||650))}}
async function resetReport(){if(location.protocol==="file:"){if(mockReplay)window.location.reload();else toast("请通过本地演示服务执行重置");return}evaluationActive=false;if(pollTimer)clearInterval(pollTimer);workflowStop();clearAutoFocusPause();pending=[];pendingKeys.clear();processing=false;newCardIndexes.clear();try{const response=await fetch(new URL("../../api/reset",document.baseURI),{method:"POST"});if(!response.ok)throw new Error("reset failed");const view=await response.json();state=view.items.map((item,index)=>({...item,status:"待开始",index,binding:{...(item.binding||{}),evidence:[...((item.binding||{}).evidence||[])]},detail:{...(item.detail||{}),state:"locked",checks:[]}}));current=0;activeAnalysisIndex=0;filter="all";document.querySelectorAll(".chip").forEach(x=>x.classList.toggle("active",x.dataset.filter==="all"));closeDetail();$("start").textContent="▶ 启动评测";$("connection").textContent=liveRemoteStream?"实时视频已连接":"等待视频接入";render();toast("已恢复初始评测状态")}catch(error){if(mockReplay){window.location.reload();return}toast("重置失败，请检查本地演示服务")}}
...
window.addEventListener("beforeunload",closeLiveVideoInput);
</script>
</body></html>'''
...
def render_html(payload: Mapping[str, Any]) -> str:
    ...
    data = data.replace("</", "<\\/")
    media_mode = str(public.get("presentation", {}).get("evidence_media_mode") or "image")
    sample_src = "../../demo-media/live-input-sample.webm"
    if media_mode == "video":
        sample_path = Path(__file__).resolve().parent / "demo-media" / "live-input-sample.webm"
        sample_src = "data:video/webm;base64," + base64.b64encode(sample_path.read_bytes()).decode("ascii")
    output = (
        HTML_TEMPLATE
        .replace("__REPORT_DATA__", data)
        .replace("__LOCAL_WEBRTC_SAMPLE__", sample_src)
    )
...
def main() -> int:
    ...
    if args.command == "template":
        _write_json(
            args.output,
            template_payload(
                load_tool_profile(args.profiles) if args.profiles else None,
