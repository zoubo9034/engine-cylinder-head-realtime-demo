...
async function startEvaluation(){if(evaluationActive){evaluationActive=false;if(pollTimer)clearInterval(pollTimer);workflowStop();pauseEvidencePlayers();$("start").textContent="▶ 启动评测";$("connection").textContent="评测已暂停";toast("实时评测已暂停");return}const fileMode=location.protocol==="file:";if(fileMode&&(evidenceMediaMode==="video"||!mockReplay)){toast("请通过本地演示服务启动视频证据展示");return}if(!await videoServiceReady()){$("connection").textContent="视频服务未就绪";toast("请检查视频证据服务");return}evaluationActive=true;$("start").textContent="Ⅱ 暂停评测";$("connection").textContent="实时分析中";toast("已启动实时评测");if(mockReplay){runMockEvents()}else if(!fileMode){await pollReport();pollTimer=setInterval(pollReport,Number(DATA.presentation.poll_interval_ms||650))}}
