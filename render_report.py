...
function pauseEvidencePlayers(except=null){document.querySelectorAll("video[data-evidence-video]").forEach(video=>{if(video!==except&&!video.paused)video.pause()});if(!except)activePlayingVideo=null}
...
function bindEvidenceVideo(video){if(video.dataset.playerBound==="true")return;video.dataset.playerBound="true";video.addEventListener("pointerdown",()=>selectEvidenceVideo(video));video.addEventListener("play",()=>{selectEvidenceVideo(video);pauseEvidencePlayers(video);activePlayingVideo=video;videoPlaybackStatus(video)});video.addEventListener("loadeddata",()=>videoPlaybackStatus(video));video.addEventListener("error",()=>videoPlaybackStatus(video,"视频加载失败，请确认服务已加载视频 Mock 报告"))}
function requestEvidencePlay(video){if(!video||!video.src)return;bindEvidenceVideo(video);videoPlaybackStatus(video,"正在加载视频…");if(video.readyState===0)video.load();const result=video.play();if(result&&typeof result.catch==="function")result.catch(error=>{videoPlaybackStatus(video,error&&error.name==="NotAllowedError"?"请点击播放器中的播放按钮":"视频加载失败，请确认服务已加载视频 Mock 报告")})}
