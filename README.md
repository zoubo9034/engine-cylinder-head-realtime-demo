...
主区顶部预留 16:9 实时视频窗口，当前仅显示接入占位，不加载历史图片或模拟画面，后续可通过窗口内的 `video` 元素接入视频流。13 个评分卡在项目从“待开始”进入“已定位”后才出现；项目完成评分时页面自动将最新完成卡片平滑滚动到视口中央，减少现场查找操作。

视频窗口下方的“模型工作状态”跑灯在“证据生成中”时启动，按接入画面、任务规划、工具编排、视觉分析、证据整理、结果判定六个高层阶段循环点亮。阶段时长依据真实 workflow trace 的工作量设置不同权重，并在每轮播放加入受限随机变化；进入终态后立即停止。标准现场页和 mock 回放页使用同一套展示逻辑，跑灯不参与评分。
